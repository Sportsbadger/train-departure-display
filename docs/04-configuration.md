...
| `modeSwitchInterval` | `300` (seconds per display mode when ADS-B is enabled; the board alternates train mode and aircraft mode at this interval)
| `adsbEnabled` | `True` (enables alternating ADS-B aircraft mode)
| `adsbHost` | `192.168.1.50` (hostname/IP of the Raspberry Pi or container exposing Beast binary data)
| `adsbPort` | `30005` (TCP Beast binary output port; `readsb`/`ultrafeeder` commonly expose Beast data here)
| `adsbReceiverLatitude` | `51.501` (receiver antenna latitude; required when `adsbEnabled=True` for nearest-aircraft sorting)
| `adsbReceiverLongitude` | `-0.142` (receiver antenna longitude; required when `adsbEnabled=True` for nearest-aircraft sorting)
| `adsbReadTimeout` | `2.0` (seconds to spend reading Beast data during each aircraft refresh)
| `adsbMaxAircraftAge` | `300` (seconds before an aircraft is dropped if no fresh ADS-B messages arrive)
| `adsbMaxPlanes` | `6` (maximum number of nearest aircraft to keep in the aircraft display rotation)
