...
| `modeCycleCount` | `1` (complete item-list cycles to show before switching modes; trains count one pass through listed trains, ADS-B counts one pass through displayed aircraft)
| `modeSwitchInterval` | `300` (optional seconds before switching to the next configured mode; when set, this overrides `modeCycleCount`)
