...
| `planeAlertMaxAgeMinutes` | `10` (maximum alert age in minutes; set blank to show all Plane-Alert history; `planeAlertMaxAge` is accepted as a minutes alias)
...
The Plane-Alert board sorts alerts newest first using the latest available observed timestamp such as `last_seen` before falling back to `timestamp` or `first_seen`, then mirrors the ADS-B layout as closely as the Plane-Alert data allows: callsign on the highlighted left block, tail/hex, equipment, and observed time on the highlighted right block, equipment/owner/position/time in the scrolling detail, and secondary rows with rank, callsign, and equipment on the left plus tail/hex and observed time on the right. Like ADS-B mode, the highlighted full-detail Plane-Alert record cycles through all displayed aircraft using a slightly extended `loopDepartureInterval`; the lower rows show the following records and then the configured `lastLineText` centered at the end of the list. The Plane-Alert board also labels the clock row with `Plane Alert`. Plane-Alert JSON is refreshed in the background and cached before/while the mode is displayed, so slow history queries do not block OLED animation or mode transitions. The app sends no-cache headers on Plane-Alert fetches and filters records older than the 10-minute default `planeAlertMaxAgeMinutes`, so an all-history `planeAlertSourceUrl` does not keep showing stale morning alerts. Existing `planeAlertMaxAgeHours` settings are still honoured for compatibility, including a blank value to disable filtering, but new configs should use minutes. The example `planeAlertSourceUrl` queries all timestamps; for large Plane-Alert histories, prefer a narrower docker-planefence regex query or increase `planeAlertFetchTimeout`.
