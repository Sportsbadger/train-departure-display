...
def test_parse_aircraft_payload_normalizes_fields():
...
def test_parse_aircraft_payload_requires_aircraft_list():
    with pytest.raises(ValueError, match="aircraft list"):
        parse_aircraft_payload({"now": 123})


def test_prepare_display_aircraft_filters_and_sorts_nearest():
    aircraft = parse_aircraft_payload(
        {
            "aircraft": [
                ...
                {"hex": "nopos", "seen": 1},
                {"hex": "near", "flight": "N123AB", "lat": 51.501, "lon": -0.12, "seen": 1},
            ]
        }
    )
...
def test_format_aircraft_rows_are_compact():
    aircraft = parse_aircraft_payload(
        {
            "aircraft": [
                {
...
def test_compass_label_wraps_north():
    assert compass_label(359) == "N"
    assert compass_label(45) == "NE"
    assert compass_label(181) == "S"


def test_build_adsb_json_url_prefers_explicit_url():
    assert build_adsb_json_url({"adsb": {"jsonUrl": "http://feed/aircraft.json"}}) == "http://feed/aircraft.json"


def test_build_adsb_json_url_from_host():
    config = {
        "adsb": {
            "jsonUrl": "",
            "host": "192.0.2.10",
...
def test_adsb_source_poll_keeps_cached_data_on_failure(monkeypatch):
