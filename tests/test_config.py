...
def test_adsb_config_defaults_to_disabled_train_only(monkeypatch):
    ...
    assert config["transport"]["modeCycleCount"] == 1
    assert config["transport"]["modeSwitchInterval"] == 300
    assert config["transport"]["modeSwitchIntervalSet"] is False
...
def test_adsb_config_parses_enabled_values(monkeypatch):
    ...
    monkeypatch.setenv("modeCycleCount", "2")
    ...
    assert config["transport"]["modeCycleCount"] == 2
    assert config["transport"]["modeSwitchInterval"] == 60
    assert config["transport"]["modeSwitchIntervalSet"] is True
